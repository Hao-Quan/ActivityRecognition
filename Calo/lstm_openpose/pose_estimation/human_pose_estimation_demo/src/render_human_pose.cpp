...
void renderHumanPose(const std::vector<HumanPose>& poses, cv::Mat& image,
    ...
    for (const auto& pose : poses) {
        ...
        for (size_t keypointIdx = 0; keypointIdx < pose.keypoints.size(); keypointIdx++) {
            if (pose.keypoints[keypointIdx] != absentKeypoint) {
                ...
                if (show_keypoints_index) {
                  cv::String str_index(std::to_string(keypointIdx));
                  cv::putText(image, str_index, pose.keypoints[keypointIdx],
                              cv::FONT_HERSHEY_COMPLEX_SMALL, 2, cv::Scalar(0,0,255));
                }

                if (show_x) {
                  std::stringstream x_stream;
                  x_stream << std::fixed << std::setprecision(0);
                  x_stream << pose.keypoints[keypointIdx].x;
                  cv::String str_x(x_stream.str());
                  cv::putText(image, str_x, pose.keypoints[keypointIdx],
    ...
    for (const auto& pose : poses) {
        for (const auto& limbKeypointsId : limbKeypointsIds) {
            ...
            int color_index = limbKeypointsId.second;
            if (pose.id != -1)
              color_index = pose.id % 18;
            cv::fillConvexPoly(pane, polygon, colors[color_index]);
        }
    }
